...
async function subscribe()
{
	...
	//const channel = `pch-mb-comment-${ testWalletObjList.alice.address }`;
	const channels = [
		`pch-mb-post-${ testWalletObjList.alice.address }`,
		`pch-mb-comment-${ testWalletObjList.alice.address }`,
		`pch-mb-like-${ testWalletObjList.alice.address }`,
		`pch-mb-favorite-${ testWalletObjList.alice.address }`,
		`pch-mb-follower-${ testWalletObjList.alice.address }`,
	];
	...
	for ( const channel of channels )
	{
		...
		subscribeRequest.sig = await Web3Signer.signObject( testWalletObjList.alice.privateKey, subscribeRequest );
		...
		console.log( `🚜 Client : server response of the subscription request to channel [${ channel }]: `, subscribeResponse );
